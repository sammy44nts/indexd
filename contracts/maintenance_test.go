...
func TestWalletMaintenance(t *testing.T) {
	...
	hm, err := hosts.NewManager(s, nil, store, hosts.WithLogger(log.Named("hosts")))
	if err != nil {
		t.Fatal(err)
	}
	defer hm.Close()

	contracts, err := contracts.NewManager(sk, nil, cm, store, nil, hm, s, w, contracts.WithLogger(log.Named("contracts")), contracts.WithSyncPollInterval(250*time.Millisecond))
	if err != nil {
		t.Fatal(err)
	}
