...
func TestWalletMaintenance(t *testing.T) {
	...
	contracts, err := contracts.NewManager(sk, nil, nil, cm, store, nil, nil, hm, s, w, contracts.WithLogger(log.Named("contracts")), contracts.WithSyncPollInterval(250*time.Millisecond))
	if err != nil {
		t.Fatal(err)
	}
